...
class RevoltSternThrusterParams: # back
    ...
    speed_max: float = math.sqrt(f_max/k_pos)                   # Max (positive) propeller speed
    speed_min: float = -math.sqrt(-f_min/k_neg)
    alpha_max: float = math.pi / 2                                     # Max (positive) propeller speed
    alpha_min: float = -math.pi / 2                                    # Min (negative) propeller speed
    max_radians_per_step: float = math.pi/6
    max_newton_per_step: float = 10
    geometry = THRUSTER_GEOMETRY(THRUSTER_LENGTH_DEFAULT, THRUSTER_WIDTH_DEFAULT)
...
class RevoltParameters3DOF:
    ...
    mean_height_above_water: float = 0.5 
...
class ReVolt3Dynamics(IDynamics):
    ...
    def continuous_time_dynamics(self, x: cs.SX, u: cs.SX, theta: cs.SX, disturbance: cs.SX, *args, **kwargs) -> cs.SX:
        ...
        dp_mask = cs.DM([1.0, 1.0, 1.0 if self.dp_mode else 0.0])

        # When DP mode is disabled, force bow azimuth/speed setpoints to zero and remove bow thrust.
        azimuth_setpoint = azimuth_setpoint * dp_mask
        speed_setpoint = speed_setpoint * dp_mask
        propeller_effectiveness = propeller_effectiveness * dp_mask

        # Generalized force generated by actuators
        thrust = propeller_effectiveness * self.actuators_params.k_pos * thruster_speed * cs.fabs(thruster_speed)
...
class ReVolt3(IVessel):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            ReVolt3Dynamics(dt, dp_mode=dp_mode),
    ...
    def rollout_dynamics_batch(
            ...
            disturbance: Optional[npt.NDArray] = None,
                method: Literal['loop', 'mapaccum'] = 'loop',
    ) -> npt.NDArray:
        """
        Fast batched dynamics rollout for sampling-based planning and dataset generation.

        x0:             initial states               (N, nx)
        U:              control sequence             (N, nu, T)
        theta:          fault parameters             (N, nt), defaults to ones
        disturbance:    external disturbance         (N, nd), defaults to zeros
        method:         rollout backend              ('loop' or 'mapaccum')
        ...
        if theta is None:
            theta = np.ones((N, self.dynamics.nt))
        if disturbance is None:
            disturbance = np.zeros((N, self.dynamics.nd))

        return self.dynamics.rollout_batch(x0, U, theta, disturbance, method=method)
    ...
    def __plot__(self, ax, *args, verbose:int=0, **kwargs):
        ...
        for i in range(3) if self.dp_mode else range(2):
...
if __name__ == "__main__":
    ...
    n_samples = 100
    horizon = 30
    noise_std = np.array([np.pi, np.pi, np.pi, 100.0, 100.0, 100.0])
    t_grid = np.arange(0.0, 10.0 + 1e-12, dt)

    def nominal_command(t: float) -> npt.NDArray:
        return np.array([np.pi/2, np.pi/2, np.pi/2 * np.sin(2*np.pi*0.02*t), 200, 200, 200])

    def run_simulation(method: Literal['loop', 'mapaccum'], plot: bool = False, seed: int = 123) -> float:
        rng = np.random.default_rng(seed)
        vessel = ReVolt3(dt, nu=(0.7, 0, 0), thruster_speeds=(200, 200, 200))
        u_min = np.concatenate([vessel.actuator_params.alpha_min, vessel.actuator_params.speed_min])
        u_max = np.concatenate([vessel.actuator_params.alpha_max, vessel.actuator_params.speed_max])
        fig, ax = None, None
        ...
        rollout_elapsed = 0.0
        for t in t_grid:
            nominal_u = nominal_command(float(t))
            vessel.step(None, None, [], [], control_commands=nominal_u)

            x0 = np.tile(vessel.states[None, :], (n_samples, 1))
            U = np.tile(nominal_u[None, :, None], (n_samples, 1, horizon))
            U += rng.normal(0.0, noise_std[None, :, None], size=U.shape)
            U = np.clip(U, u_min[None, :, None], u_max[None, :, None])

            t0 = time.perf_counter()
            X = vessel.rollout_dynamics_batch(x0, U, method=method)
            ...
            if plot:
                ...
                for i in range(n_samples):
                    ax.plot(X[i, 1, :], X[i, 0, :], color='0.6', alpha=0.35, linewidth=1.0)
                vessel.plot(ax=ax)
                ax.set_title(f"{method} | t={t:.1f}")
                fig.canvas.draw()
                fig.canvas.flush_events()

        return rollout_elapsed / (len(t_grid) * n_samples)

    revolt = ReVolt3(dt, nu=(0.7, 0, 0), thruster_speeds=(200, 200, 200))
    ...
    avg_loop = run_simulation('loop', plot=False, seed=123)
    avg_mapaccum = run_simulation('mapaccum', plot=False, seed=123)
    best_method: Literal['loop', 'mapaccum'] = 'loop' if avg_loop <= avg_mapaccum else 'mapaccum'

    print(f"Average rollout time per step (loop): {avg_loop*n_samples:.4f} s")
    print(f"Average rollout time per step (mapaccum): {avg_mapaccum*n_samples:.4f} s")
    print(f"Best method: {best_method}")

    run_simulation(best_method, plot=True, seed=123)