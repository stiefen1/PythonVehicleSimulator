...
# from control.utils import is_controllable, is_observable

# def extract_active_states_matrices(A, B):
#     """
#     Extract submatrices corresponding to active states only.
    
#     Active states for ReVolt 3DOF:
#     - eta: [0, 1, 5] (x, y, psi)
#     - nu:  [6, 7, 11] (u, v, r) 
#     - azimuth: [12, 13, 14]
#     - thruster_speeds: [15, 16, 17]
    
#     Unused states: [2, 3, 4, 8, 9, 10] (z, phi, theta, w, p, q)
#     """
#     active_indices = [0, 1, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17]
    
#     # Extract submatrices for active states only
#     A_active = A[np.ix_(active_indices, active_indices)]
#     B_active = B[np.ix_(active_indices, list(range(B.shape[1])))]
...
class RevoltThrusterParameters:
    ...
    u_min: npt.NDArray = field(init=False)                      
    u_max: npt.NDArray = field(init=False)                      
    ...
    def __post_init__(self):
        ...
        self.u_min = np.concatenate([self.alpha_min, self.speed_min]).squeeze()
        self.u_max = np.concatenate([self.alpha_max, self.speed_max]).squeeze()
...
class ReVolt3Dynamics(IDynamics):
    ...
    def __init__(
        ...
        self._g: cs.Function = self._get_g()

    def actuators_force(self, u_actual: cs.SX, theta: cs.SX, *args, **kwargs) -> cs.SX:
        """
        Compute generalized force generated by actuators
        """
        azimuth_stucked, propeller_effectiveness = theta[0:3], theta[3:6]
        azimuth, thruster_speed = u_actual[0:3], u_actual[3:6]
    ...
    def _get_g(self) -> cs.Function:
        """
        Creates CasADi function for actuators force.
        
        Returns:
            cs.Function: actuators force
        """
        u_actual = cs.SX.sym('u_actual', self.nu)                     # type: ignore
        theta = cs.SX.sym('theta', self.nt)             # type: ignore

        return cs.Function('g', [u_actual, theta], [self.actuators_force(u_actual, theta)])

    def g(self, u_actual: npt.NDArray, theta: npt.NDArray) -> npt.NDArray:
        """
        Generalized force generated by the actuators
        """
        return np.array(self._g(u_actual, theta))
    ...
    def continuous_time_dynamics(self, x: cs.SX, u: cs.SX, theta: cs.SX, disturbance: cs.SX, *args, **kwargs) -> cs.SX:
        ...
        azimuth_setpoint = u[0:3] # cs.fmax(cs.fmin(u[0:3], self.actuators_params.alpha_max), self.actuators_params.alpha_min)
        speed_setpoint = u[3:6] # cs.fmax(cs.fmin(u[3:6], self.actuators_params.speed_max), self.actuators_params.speed_min)
        azimuth_stucked, propeller_effectiveness = theta[0:3], theta[3:6]

        # Generalized force generated by actuators
        # thrust = propeller_effectiveness * self.actuators_params.k_pos * thruster_speed * thruster_speed
        # tau_actuators = self.actuators_params.Alpha(azimuth[0], azimuth[1], azimuth[2]) @ thrust
        tau_actuators = self.actuators_force(x[12:18], theta)
...
class ReVolt3(IVessel):
    ...
    def __dynamics__(self, control_commands:npt.NDArray, current:Current, wind:Wind, *args, theta: Optional[npt.NDArray] = None, **kwargs) -> np.ndarray:
        ...
        feasible_control_commands = np.clip(control_commands, self.actuator_params.u_min, self.actuator_params.u_max)
        x = self.dynamics.fd(self.states, feasible_control_commands, theta, disturbance).flatten()

        # A = self.dynamics.Ad(self.states, control_commands, theta, disturbance)
        # B = self.dynamics.Bd(self.states, control_commands, theta, disturbance)
        
        # # Extract only active states for controllability analysis
        # A_active, B_active = extract_active_states_matrices(A, B)
        # print("is controllable ? ", is_controllable(A_active, B_active), 
        #       f"Full system: {A.shape}, Active system: {A_active.shape}")
        # C = np.eye(*A_active.shape)
        # C[6:12, 6:12] = 0
        # print(C)
        # print("is observable ? ", is_observable(A_active, C))
...
if __name__ == "__main__":
    ...
    revolt = ReVolt3(0.1, nu=(0.7, 0, 0), thruster_speeds=(0, 20, 20), azimuth_angles=(np.pi/4, np.pi/2, 0))
    ...
    for t in np.linspace(0, 100, int(100//0.1)+1):
        revolt.step(None, None, [], [], control_commands=np.array([np.pi/2, np.pi/2, np.pi/2 * np.sin(2*np.pi*0.2*t), 0, 20, 20]))
        ax.cla()
        ax.set_xlim(-10, 10)
        ax.set_ylim(-10, 10)
        ax.set_aspect('equal')
